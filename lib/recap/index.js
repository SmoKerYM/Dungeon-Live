 ...
 *       → 转写全文 + 人名对照表 + 世界观背景交给 DeepSeek 总结 → 并进已有的前情提要
 *       → 广播给所有人。DM 之后可以随意手改，玩家只能看。
 *
 ...
 *     recap:start / recap:stop         DM：开始 / 结束录制（点一下开始、再点一下结束，不用按住）
 *     recap:chunk   ArrayBuffer        DM：一帧 16kHz / PCM16 LE / 单声道音频
 *     recap:cancel                     DM：放弃这次录制，什么都不总结
 *     recap:retry                      DM：上次总结失败了，用存下来的转写再总结一次
 *     recap:update  { brief, people }  DM：手改后的整份内容
 *     recap:text    { text }           DM 调试：跳过 ASR 直接拿文本去总结
 *   Server → Client
...
function createRecap({ io, filePath, deepseek, asr }) {
  ...
  async function runSummary(text, notice) {
    ...
    try {
      const summary = await summarizeRecap(text, store.get(), deepseek);
      store.setPendingTranscript('');
      if (!summary.brief && !summary.people.length) {
        setPhase('idle', '没从讲述里听出剧情，请再录一次');
        return;
      }
      store.applySummary(summary);
      io.emit('recap:sync', syncPayload());
      setPhase('idle', notice);
    } catch (err) {
