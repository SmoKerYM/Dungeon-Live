...
const {
  computeRollModifier, buildRollOutcome, buildRollLabel, buildRollExpr, resolveAttackAbility
} = require('./dice');
...
function sendPreview(socket, deps, player, intent, transcript) {
  ...
  // 命中投的 label 要标出实际用的属性，而 finesse 用哪个取决于角色卡
  const attackAbility = intent.type === 'attack' ? resolveAttackAbility(card, intent.key) : null;
...
function rollAndBroadcast(io, socket, deps, player, intent) {
  ...
  const modifier = computeRollModifier(card, intent);
  const attackAbility = intent.type === 'attack' ? resolveAttackAbility(card, intent.key) : null;
  const outcome = buildRollOutcome(player, intent, modifier, attackAbility);
