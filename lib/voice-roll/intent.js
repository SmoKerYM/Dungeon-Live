 ...
 *       「哪一类检定（含命中投）× 哪一项 × 是否优劣势」；判不出来就返回 null，交给 LLM 兜底。
...
/**
 * 判断命中投用的是哪一类武器（plan §3.2 的 WEAPON_CLASSES）。
 *
 * 匹配**长词优先**：「撬棍」必须盖过「棍」，否则撬棍会被当成棍棒类、白加一个熟练。
 * 被更长的匹配完全覆盖的短词会被丢掉；剩下的匹配若分属不同类别，
 * 说明一句话里提了两种武器（「掏枪还是用刀」），规则不猜，交给 LLM。
 *
 * @param {string} compact 去空格文本（调用方已去掉「一刀」这类量词）
 * @param {string} spaced  带空格文本
 * @returns {string|null} WEAPON_CLASSES 的 key；没提武器返回 'none'；有歧义返回 null
 */
function detectWeaponClass(compact, spaced) {
  const hits = [];
  for (const [cls, def] of Object.entries(V.WEAPON_CLASSES)) {
    for (const w of def.words) {
      const i = compact.indexOf(w);
      if (i !== -1) hits.push({ cls, at: i, len: w.length });
    }
    for (const w of def.en) {
      const m = new RegExp(`\\b${escapeRe(w)}\\b`).exec(spaced);
      if (m) hits.push({ cls, at: m.index, len: w.length, en: true });
    }
  }
  if (hits.length === 0) return 'none';

  // 丢掉被更长匹配覆盖住的短词（中文与英文各自在自己的坐标系里比较）
  const kept = hits.filter(h => !hits.some(o =>
    o !== h && !!o.en === !!h.en && o.len > h.len &&
    o.at <= h.at && o.at + o.len >= h.at + h.len));

  const classes = new Set(kept.map(h => h.cls));
  if (classes.size > 1) return null;
  return kept[0].cls;
}

/**
 * 判断玩家有没有口头覆盖「加不加熟练」。先查否定再查肯定——
 * 「不加熟练」里本身就包含「加熟练」。
 *
 * @param {string} compact 去空格文本
 * @param {string} spaced  带空格文本
 * @returns {boolean|null} true=明说加，false=明说不加，null=没说（用武器默认）
 */
function detectProficiencyOverride(compact, spaced) {
  if (indexOfAnyCn(compact, V.PROFICIENCY_NO_CN) !== -1 ||
      indexOfAnyEn(spaced, V.PROFICIENCY_NO_EN) !== -1) return false;
  if (indexOfAnyCn(compact, V.PROFICIENCY_YES_CN) !== -1 ||
      indexOfAnyEn(spaced, V.PROFICIENCY_YES_EN) !== -1) return true;
  return null;
}

/**
 * 把熟练短语从文本里剥掉。
 * 必须在检查 EXTRA_MOD_HINTS 之前做：「熟练加值」里含「加值」，
 * 不剥掉的话「命中投，熟练加值」会被当成玩家口述的额外加减值、整句踢给 LLM。
 *
 * @param {string} text
 * @returns {string}
 */
function stripProficiencyPhrases(text) {
  let out = text;
  for (const w of [...V.PROFICIENCY_NO_CN, ...V.PROFICIENCY_YES_CN,
                   ...V.PROFICIENCY_NO_EN, ...V.PROFICIENCY_YES_EN]) {
    if (out.includes(w)) out = out.split(w).join(' ');
  }
  return out;
}
 ...
 *              weaponClass?: string, proficiencyOverride?: boolean|null }}
 *          null → 规则判不了，调用方应转 LLM 兜底；
 *          { error } → 明确判错且不该猜（说了「豁免」却没说哪一项、或者要投伤害），直接回 voice:error；
 *          其余 → 可直接使用的意图。
 */
function parseRollIntent(text) {
  ...
  // 熟练覆盖要在剥离之前判断（剥完就看不见了）；它只对命中投有意义
  const proficiencyOverride = detectProficiencyOverride(compact, spaced);
  // 剥掉熟练短语再查额外加减值：「熟练加值」里含「加值」，不剥的话整句会被踢给 LLM
  const spacedNoProf = stripProficiencyPhrases(spaced);
  const compactNoProf = spacedNoProf.replace(/\s+/g, '');

  // 玩家明说了额外加减值（「再减二」）时规则层不接，整句交给 LLM，
  // 免得把这个修正值悄悄吃掉
  if (indexOfAnyCn(compactNoProf, V.EXTRA_MOD_HINTS) !== -1) return null;
  ...
  // 2. 伤害骰：第一版不支持。必须排在攻击判定之前，
  //    否则「攻击伤害」会被当成命中投投出去
  if (indexOfAnyCn(compact, V.DAMAGE_CN) !== -1 ||
      indexOfAnyEn(spaced, V.DAMAGE_EN) !== -1) {
    return { error: '暂不支持伤害骰' };
  }

  // 3. 命中投：有明确的攻击触发词才接，只描述动作的（「砍他一刀」）交给 LLM
  if (indexOfAnyCn(compact, V.ATTACK_TRIGGER_CN) !== -1 ||
      indexOfAnyEn(spaced, V.ATTACK_TRIGGER_EN) !== -1) {
    // 「一刀」「两刀」是量词不是武器，匹配刀类之前先去掉
    const weaponText = compactNoProf.replace(V.KNIFE_QUANTIFIER_RE, '');
    const weaponClass = detectWeaponClass(weaponText, spacedNoProf.replace(V.KNIFE_QUANTIFIER_RE, ''));
    if (!weaponClass) return null;   // 一句话里提了两类武器，规则不猜

    // key 和「加不加熟练」都由 WEAPON_CLASSES 派生，规则层不自己定
    const intent = done('attack', V.WEAPON_CLASSES[weaponClass].key);
    intent.weaponClass = weaponClass;
    intent.proficiencyOverride = proficiencyOverride;
    return intent;
  }

  // 4. 技能（属性名与技能名同时出现时取技能）
...
function validateIntent(obj) {
  ...
  // 命中投：武器类别与熟练覆盖，非命中投时一律忽略
  const weaponClass = type === 'attack'
    ? (obj.weaponClass == null ? 'none' : String(obj.weaponClass)) : null;
  const rawOverride = obj.proficiencyOverride;
  if (type === 'attack' && rawOverride != null && typeof rawOverride !== 'boolean') return null;
  const proficiencyOverride = type === 'attack' && typeof rawOverride === 'boolean'
    ? rawOverride : null;
  ...
  if (type === 'ability' || type === 'save') {
    ...
    // LLM 给的 key 一律不信：按 weaponClass 查表派生（plan §7）
    if (!V.WEAPON_CLASS_KEYS.includes(weaponClass)) return null;
    key = V.WEAPON_CLASSES[weaponClass].key;
  } else {
  ...
  const out = { type, key, advantage, extraModifier, confidence, note };
  if (type === 'attack') {
    out.weaponClass = weaponClass;
    out.proficiencyOverride = proficiencyOverride;
  }
  return out;
}

module.exports = {
  parseRollIntent, validateIntent, normalizeText, detectAdvantage,
  detectWeaponClass, detectProficiencyOverride, stripProficiencyPhrases
};
