...
/**
 * 判断命中投用的是哪种武器模式。
 * 没提到任何武器（包括只说了「近战」）一律按 finesse 处理——
 * 本桌规则是取力量和敏捷中较高的那个（现代团最常用刀）。
 *
 * @param {string} compact 去空格文本
 * @param {string} spaced  带空格文本
 * @returns {'melee'|'ranged'|'finesse'|null} null 表示远程词和近战词同时出现，规则不猜，交给 LLM
 */
function detectAttackMode(compact, spaced) {
  const ranged = indexOfAnyCn(compact, V.ATTACK_RANGED_CN) !== -1 ||
                 indexOfAnyEn(spaced, V.ATTACK_RANGED_EN) !== -1;
  const melee = indexOfAnyCn(compact, V.ATTACK_MELEE_CN) !== -1 ||
                indexOfAnyEn(spaced, V.ATTACK_MELEE_EN) !== -1;
  const finesse = indexOfAnyCn(compact, V.ATTACK_FINESSE_CN) !== -1 ||
                  indexOfAnyEn(spaced, V.ATTACK_FINESSE_EN) !== -1;

  if (ranged && (melee || finesse)) return null;
  if (ranged) return 'ranged';
  if (melee && finesse) return null;   // 「用刀背当锤子砸」这种也别猜
  if (melee) return 'melee';
  return 'finesse';                    // 灵巧武器，或压根没提武器
}
...
function parseRollIntent(text) {
  ...
  if (indexOfAnyCn(compact, V.ATTACK_TRIGGER_CN) !== -1 ||
    ...
    const mode = detectAttackMode(compact, spaced);
    if (!mode) return null;   // 远近武器词同时出现，规则不猜
    return done('attack', mode);
  }
...
function validateIntent(obj) {
  ...
  if (type === 'ability' || type === 'save') {
    ...
    if (!key || !V.ATTACK_MODES.includes(key)) return null;
  } else {
