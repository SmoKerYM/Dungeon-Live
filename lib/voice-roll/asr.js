 ...
 * @param {string} [cfg.contextPrompt] 热词上下文，默认是掷骰黑话（ASR_CONTEXT_PROMPT）；
 *        前情提要（lib/recap）传的是本团的人名表
 ...
 *        err.partialText 带着出错前已经识别出的文本（长录音不至于前功尽弃）
...
function createAsrSession(cfg, handlers) {
  ...
  /** 已经定稿的句子（sentence_end=true 的那些），按句号顺序拼起来 */
  function finalizedText() {
    const ids = [...sentences.keys()].sort((a, b) => a - b);
    return ids.map(id => sentences.get(id)).join('');
  }

  /** 定稿部分 + 当前这句还没说完的部分 —— partial 显示和最终结果都用它 */
  function joinedText() {
    return (finalizedText() + lastPartial).trim();
  }
  ...
  ws.on('open', () => {
    ws.send(JSON.stringify({
      ...
      payload: {
        ...
        input: {
          context: [
            { role: 'user', content: [{ type: 'input_text', text: cfg.contextPrompt || ASR_CONTEXT_PROMPT }] }
          ]
        }
      }
    }));
