...
- **血条 HUD** - 屏幕顶部正中的扁平胶囊血条，绑定当前载入的角色卡（玩家进场自动载入自己的，DM 跟着他在角色卡面板打开的那张）。血量高于 50% 绿、25~50% 黄、低于 25% 红。点血条就地变成输入框改「当前血量 / 生命上限」；玩家另有 ± 按钮各增减 1 点（只改当前血量，改上限仍需点血条）。改动实时广播，玩家列表和棋子悬浮提示同步
