...
- **液态贴墙按钮** - 屏幕左侧从上到下依次是 地图 / 玩家 / NPC（仅 DM）/ 笔记 / 角色卡；右侧是骰子。鼠标悬浮时按钮像液体一样被拉长（按钮本身不消失），同时弹出磨砂圆角浮动窗口
- **点击固定** - 点按钮即固定该窗口，鼠标移开也不再收起；再点一次取消。窗口标题栏的 📌 / ✕ 同样可用
...
- **展开式聊天室** - 「聊天」标题旁 `Click Me` 展开覆盖玩家列表 + 聊天框的气泡界面：玩家以自选颜色圆形头像标识，自己的消息靠右、他人靠左、系统消息居中，右上角 ✕ 关闭
