...
- 2026-09-26: 角色卡 AI 一句话总结（DeepSeek）+ 数字输入框箭头放大
  ...
  - AI 总结区放在角色名**下方通栏**——右侧只有约 200px，装不下 80 字的一句话
  - **服务端代理**：`DEEPSEEK_API_KEY` 只存在于服务端环境变量，客户端永远拿不到（key 涉及真实费用，不能走前端）。`.env` 已进 `.gitignore`，另有 `.env.example` 模板；启动脚本用 Node 内置的 `--env-file-if-exists=.env`
  - 模型选 `deepseek-flash` + `thinking: {type:'disabled'}`：这个任务只是读几个数字写一句话，v4-pro 输入贵 4 倍输出贵 3 倍；flash 默认开思考会白烧输出 token
  - 指纹缓存：`characterFingerprint()` 只哈希影响结论的字段，改血量不触发重新生成；结果存进角色卡的 `aiSummary`
  - Prompt 里交代世界观（2000 年前后现代社会、普通人、无魔法）并**把技能翻译成"具体能做什么"**，这样输出才会是"能从窗户翻进去"而不是"敏捷很高"
  - 实测：V 卡 1.5s 返回、命中缓存不调 API、强制刷新能重新生成、无专长的 Player2 按 prompt 兜底改写另一长处
- 2026-09-26: 角色卡界面重做（TODO 第 2 项）
  - 原样式是整页视图时代直接塞进浮动窗口的，信息密度低、纵向极长
  - 布局：六维属性从「一行一个 × 6 行」改成 **3 列 × 2 行**的卡片（label 在上、数值在下）；熟练加值并入顶部一行（姓名 / 血量 / 熟练加值）；豁免检定与技能改为**并排两栏**；专长独立一段
  - 样式全面对齐磨砂玻璃语言：工具栏按钮、DM 角色列表、输入框、勾选行的 hover 底色；数值做成小圆片便于扫读；`tabular-nums` 防跳动
  - 编辑态可见性：只读时输入框边框透明，进入编辑后才显出边框和深色底
  - **修 bug**：`.char-actions` 原本是 `position: fixed; bottom: 20px; right: 90px`——整页视图时代的遗留，导致「编辑/保存」浮在屏幕右下角而不是面板里。改为 `static` 并入面板底部
  - `#panel-character` 宽度 680 → 620；所有 JS 依赖的 id / class 全部保留（已逐一核对）
