...
- **共享笔记的接线是承重墙，别再删**：左侧 textarea 的 `oninput` → `notes:update`（500ms 防抖）和 `socket.on('notes:sync')` 缺一不可。这两段曾在 `93db83e` 的全屏重构里被整段删掉，笔记框退化成只读一次的本地草稿纸——`joinSuccess` 照样灌内容进来，界面毫无异常，但写进去的字既不落盘也不同步；服务器一重启（每次部署）客户端重连，`joinSuccess` 就用服务端旧副本把浏览器里未保存的文字覆盖掉。2026-09-26 三名玩家一整天的笔记因此全部丢失、无法找回（`data/` 不在 git 里也没有备份）。改动这块之后必须实测完整往返：写 → `data/notes.txt` 变了 → 另一个客户端收到 → 刷新后还在
- 笔记的防丢三道：`pagehide` / 切后台时 `flushNotes()` 补发还压在防抖里的最后一次修改；别人的更新在你正打字时先存进 `pendingRemoteNotes`、失焦再补上（此时若自己有未发送的改动则以自己为准），两条路都保证「框里显示的」和「服务端存的」是同一份；`joinSuccess` 一律以服务端那份为准。服务端 `saveNotes()` 是**立即**写盘的，防抖只在客户端
- Notes panel split into left (shared textarea) and right (登场人物 table with name/info columns)。**两栏等高**：`#notes-view` 与两个栏都要 `min-height: 0`，人物表包在 `#char-notes-scroll` 里自己滚、`+ 添加人物` 钉在栏底 —— 少一个 `min-height: 0`，右栏就会被内容顶高、一路长出左栏底部并把整个面板撑成可滚的。表头 `position: sticky` 且背景必须**不透明**，否则钉住之后行会从底下透出来
