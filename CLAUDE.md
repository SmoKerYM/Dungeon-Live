...
- The chat input keeps a shell-style history (`chatInputHistory`, in memory, last 50, consecutive duplicates collapsed). ↑/↓ walk it and ↓ past the newest entry restores the draft the user was typing (`chatHistoryDraft`). The `@` popup claims the arrow keys first when it is open, so the two never fight. History is per tab and is lost on reload
