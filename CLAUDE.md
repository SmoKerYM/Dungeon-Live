...
│       ├── store.js           # recap.json 读写（500ms 防抖）、DM 手改的清洗、AI 总结的合并
│       ├── summarize.js       # summarizeRecap：调 DeepSeek，输出 {brief, people}
...
| Recap | `recap:fetch`（任何人）；DM 专用：`recap:start`, `recap:chunk`（ArrayBuffer，16kHz PCM16 单声道）, `recap:stop`, `recap:cancel`, `recap:retry`, `recap:update`（`{ brief, people }`）, `recap:text`（调试：跳过 ASR） |
...
- **前情提要（recap）**：左侧第五个按钮「前情」。DM 点「开始录制」（点一下开始、再点一下结束，**不用按住**）口述之前的剧情，转写**不回显**给前端；结束后把「转写 + 人名对照表（`lib/recap/checklist.json`）+ 世界观背景（`lib/recap/world-background.txt`）+ 已有前情」交给 DeepSeek，输出 `{ brief, people }`。面板左 70% 正文、右 30% 重要人物表。实现全部在 `lib/recap/` 与 `public/recap/`，`server.js` / `game.html` 只做接线（`game.html` 里只有按钮和空的面板外壳）
...
- 新的总结**追加**在已有正文之后（空一行），人物按名字合并：已有人物保留 DM 写的说明，只在说明为空时用 AI 的补上。转写 / 总结期间 DM 的编辑框只读——结果回来会整份覆盖；开始录制前会先 flush 未发送的手改
