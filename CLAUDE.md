...
- **回车要按两下才发送**（`ENTER_CONFIRM_MS` = 1500ms）：中文输入法下打英文时，回车往往只是把字母上屏，单下回车发送会把没写完的话送出去。第一下回车只「举手」（输入框描琥珀色边、右上角提示「再按一次回车发送」），1.5 秒内再按一下才真的发。组字中的回车（`e.isComposing` 或 `keyCode === 229`）直接忽略、也不算举手；一旦又打了字（`onChatInput`）、按了上下箭头翻历史、按 Esc 或失焦，举手就作废。`@` 建议框打开时回车仍然归建议框（选中名字），不进入这套确认；点「发送」按钮一下就发，不受影响
